...
from django.utils import timezone
from .utils import download_enrollment_data
...
class SectionViewSet(viewsets.ModelViewSet):
    ...
    # ──────────────────────────────────────────────────────────────
    # LIST → hanya root sections (parent=None) + prefetch semua anak + material/assessment
    # ──────────────────────────────────────────────────────────────
    def get_queryset(self):
        ...
        # Prefetch 2 level dalam + materials & assessments hanya di level Unit
        return Section.objects.filter(
            ...
            Prefetch(
                ...
                queryset=Section.objects.order_by('order').prefetch_related(
                    # Level 2: Unit → ambil materials & assessments
                    Prefetch(
                        'children',
                        queryset=Section.objects.order_by('order').prefetch_related(
                            'materials',
    ...
    # ──────────────────────────────────────────────────────────────
    # RETRIEVE, UPDATE, DELETE → tetap bisa akses semua level via PK
    # ──────────────────────────────────────────────────────────────
    def get_object(self):
    ...
    # ──────────────────────────────────────────────────────────────
    # REORDER → terima struktur baru dari frontend
    # ──────────────────────────────────────────────────────────────
    @action(detail=False, methods=['post'])
    def reorder(self, request):
        data = request.data  # list of dicts: [{id, children: [...]}, ...]
        self._update_order_recursive(data, parent_id=None)
        return Response({"status": "ok"})

    def _update_order_recursive(self, items, parent_id):
        for index, item in enumerate(items):
            section_id = item['id']
            section = get_object_or_404(Section, pk=section_id)

            # Update parent & order dalam satu save() → lebih cepat
            ...
            if item.get('children'):
                self._update_order_recursive(item['children'], parent_id=section_id)

    # ──────────────────────────────────────────────────────────────
    # DELETE → hapus + rapikan order sibling
    # ──────────────────────────────────────────────────────────────
    def destroy(self, request, *args, **kwargs):
        ...
        section.delete()  # CASCADE hapus semua anak

        # Rapikan order sibling di level yang sama
        if parent:
            siblings = parent.children.all()
        else:
            siblings = Section.objects.filter(courses=course, parent__isnull=True)

        for index, sib in enumerate(siblings.order_by('order')):
            sib.order = index
...
def course_list_enroll(request, id):
    ...
    is_partner_owner = hasattr(user, 'partner_user') and course.org_partner and course.org_partner.user == user
    is_instructor = hasattr(course, 'instructor') and course.instructor and course.instructor.user == user
    ...
    assessments = Assessment.objects.filter(section__courses=course).order_by('id')
    sections = course.sections.all()
    section_ids = sections.values_list('id', flat=True)
    user_ids = enrollments.values_list('user_id', flat=True)

    # ===== Ambil last material & assessment reads =====
    material_reads = MaterialRead.objects.filter(
        user_id__in=user_ids, material__section_id__in=section_ids
    ).order_by('-read_at')
    last_material_map = {(mr.user_id, mr.material_id): mr.read_at for mr in material_reads}

    assessment_ids = [aid for s in sections for aid in s.assessments.values_list('id', flat=True)]
    assessment_reads = AssessmentRead.objects.filter(
        user_id__in=user_ids, assessment_id__in=assessment_ids
    ).order_by('-completed_at')
    last_assessment_map = {(ar.user_id, ar.assessment_id): ar.completed_at for ar in assessment_reads}

    enrollment_details = []

    for enrollment in enrollments:
        target_user = enrollment.user
        assessment_scores = []
        ...
        for assessment in assessments:
            ...
            if lti_result and lti_result.score is not None:
                s = Decimal(lti_result.score)
                if s > 1.0:
                    s /= 100
                score_value = s * weight
                ...
                if invideo_quizzes.exists():
                    ...
                    # === CASE 4: Askora ===
                    ask_subs = Submission.objects.filter(askora__assessment=assessment, user=target_user)
                    if ask_subs.exists():
                        latest_submission = ask_subs.order_by('-submitted_at').first()
                        assessment_score = AssessmentScore.objects.filter(submission=latest_submission).first()
                        if assessment_score:
                            score_value = Decimal(assessment_score.final_score)
                            is_submitted = True
        ...
        # Progress
        progress = CourseProgress.objects.filter(user=target_user, course=course).first()
        progress_percentage = Decimal(progress.progress_percentage if progress else 0)

        # Status kelulusan
        passing_criteria_met = overall_percentage >= passing_threshold and progress_percentage == 100
        status = "Pass" if all_submitted and passing_criteria_met else "Fail"

        # Last access & last completed assessment
        last_access = max(
            [last_material_map.get((target_user.id, m.id)) for s in sections for m in s.materials.all() if last_material_map.get((target_user.id, m.id))],
            default=None
        )
        last_assessment = max(
            [last_assessment_map.get((target_user.id, a.id)) for s in sections for a in s.assessments.all() if last_assessment_map.get((target_user.id, a.id))],
            default=None
        )
        ...
        enrollment_details.append({
            ...
            'gender': getattr(target_user, 'gender', 'N/A'),
            'birth': getattr(target_user, 'birth', 'N/A'),
            'address': getattr(target_user, 'address', 'N/A'),
            'country': getattr(target_user, 'country', 'N/A'),
            'phone': getattr(target_user, 'phone', 'N/A'),
            'education': getattr(target_user, 'education', 'N/A'),
            'university': getattr(target_user, 'university', 'N/A'),
            ...
            'last_accessed_material': last_access,
            'last_completed_assessment': last_assessment,
...
def download_enrollment_data(course, enrollment_details):
    ...
    for enrollment in enrollment_details:
        ...
        birth_str = birth.strftime('%Y-%m-%d') if hasattr(birth, "strftime") else (birth if birth != 'N/A' else '')
        ...
        writer.writerow([
            ...
            f"{enrollment.get('total_score', 0):.2f} / {enrollment.get('total_max_score', 0):.2f}",
            enrollment.get('status', ''),
            f"{enrollment.get('progress_percentage', 0):.2f}%",
...
def generate_microcredential_certificate(request, id):
    ...
    # Ambil gambar microcredential dan foto author
...
# ===============================
@shared_task
def send_certificate_email(user_id, course_id, certificate_id, pdf_path, vc_path, verification_url):
    from django.contrib.auth import get_user_model
    user = get_user_model().objects.get(id=user_id)
    course = Course.objects.get(id=course_id)

    email_subject = f"Your Certificate for {course.course_name}"
    ...
    with open(pdf_path, 'rb') as f:
        email.attach(f'certificate_{course.slug}.pdf', f.read(), 'application/pdf')

    # Attach VC JSON
    with open(vc_path, 'r', encoding='utf-8') as f:
        email.attach(f'vc_certificate_{course.slug}.json', f.read(), 'application/json')
...
def generate_certificate(request, course_id):
    ...
    if not request.user.email:
        logger.warning(f"User {request.user.username} has no email")
        messages.error(request, "Your account does not have a valid email. Please update your email to claim certificate.")
        return redirect('authentication:mycourse')
    ...
    # cek pembayaran (pay-for-certificate)
    ...
    for assessment in assessments:
        ...
        if lti_result and lti_result.score is not None:
            ...
            assessment_scores.append({'assessment': assessment, 'score': score_value, 'weight': weight})
            total_score += score_value
            total_max_score += weight
            continue

        # 2. VIDEO QUIZ
        video_quiz = QuizResult.objects.filter(user=request.user, assessment=assessment).order_by("-created_at").first()
        if video_quiz:
        ...
        # 3. MCQ
        # 3. MCQ / AssessmentResult
        mcq_result = AssessmentResult.objects.filter(user=request.user, assessment=assessment).first()
        if mcq_result:
        ...
        # 4. ESSAY / SUBMISSION
        submission = Submission.objects.filter(askora__assessment=assessment, user=request.user).order_by('-submitted_at').first()
        if submission:
    ...
    # BASE URL & Partner logo
    base_url = request.build_absolute_uri('/')
    partner_logo_url = None
    if course.org_partner and getattr(course.org_partner, 'logo', None):
    ...
    try:
        ...
        logger.error(f"VC JSON generation failed: {str(e)}")
    ...
    # SEND EMAIL (jangan crash jika gagal)
    try:
        email_subject = f"Your Certificate for {course.course_name}"
        email_html = render_to_string('email/certificate_notification.html', {
            'user': request.user,
        ...
        email.send(fail_silently=False)
        logger.info(f"Certificate email sent to {request.user.email}")
    except Exception as e:
...
def create_question_view(request, idcourse, idsection, idassessment):
    user = request.user
    course = None
    team_member = None  # inisialisasi aman
    ...
    choice_formset = ChoiceFormSet(
        data=request.POST or None,
        instance=Question(),  # dummy instance untuk formset
        prefix='choices',
        form_kwargs={'assessment': assessment}  # agar formset juga tahu assessment
    )
